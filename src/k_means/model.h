...
public:
    ...
    static model* init(utils::ParsedArgs, const pointList&, int);// initialize the model randomly
    double fit(const pointList&, const pointList&);              // train KMeans using prescribed algorithm on training data
    double evaluate(const pointList&) const;                     // test KMeans according to specified method
    std::vector<unsigned> predict(const pointList&) const;       // test KMeans according to specified method
    pointList get_centers() const;
...
protected:
    ...
    std::vector<SuffStatsOne> clusters;                 // Information about the clusters
    ...
    virtual int sampling(const pointType& p) { return 0; }      // sampling on thread i outsourced to children

...
pointList get_var(const pointList& trngdata)
{
    unsigned D = trngdata.rows();
    unsigned N = trngdata.cols();
    double *data = new double[D];
    pointList initial_centres(data, D, 1);
    pointType mean = trngdata.rowwise().sum();
    pointType varDiag = trngdata.cwiseAbs2().rowwise().sum();

    double kappa = N + 0.0001;
    double nu = N + 3;

    mean = mean / kappa;

    // Update the variance
    varDiag = (varDiag - kappa * mean.cwiseAbs2());
    varDiag = (varDiag.array() + 0.003) / (3 + nu);

    // Store the inverse prior for the next iteration E step
    varDiag = 0.5 * varDiag.cwiseInverse();

    initial_centres = varDiag;
    return initial_centres;
}
...
int main(int argc, char ** argv) 
{
    ...
    pointList mean_inits = get_init_points(args.K, args.init_type, trngdata);
    pointList var_inits = get_var(trngdata);
    
    // initialize the model
    fmm = model::init(args, mean_inits, var_inits, world_rank);
    ...
    if(mean_inits.data()) delete[] mean_inits.data();
    if(var_inits.data()) delete[] var_inits.data();
    if(fmm) delete fmm;

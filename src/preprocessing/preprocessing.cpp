...
protected:
    ...
    {   
        ...
        for (auto & doc : docs )
        {
            ...
            if((d++)%1000==0)
                std::cout << "Processing document: " << d << " with " << doc.docLength << " tokens. \r" << std::flush;
            ...
            while (doc.docLength > 0)
            {
                ...
                if (word_count[doc.words[pos]] > trunc)
...
public:
    ...
    std::cout<<std::endl;
        ...
        if (argument.size() > 6)
            this->shuffle = std::stoi(argument[6]);
        else
            this->shuffle = 0;
        if (argument.size() > 7)
            this->trunc = std::stoi(argument[7]);
        else
            this->trunc = 0;
    }
...
protected:
    ...
    {    
        ...
        word_count.resize(V, 0);
        for (auto & doc : docs )
        {
            unsigned docLength = doc.words.size();
            for(unsigned i = 0; i < docLength; ++i)
                word_count[doc.words[i]] += doc.counts[i];
        }

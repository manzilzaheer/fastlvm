...
$(SOURCES): $(CURR_DIR)/src/% : $(CURR_DIR)/src/%/makefile
	@cd $@ && make $(CTYPE) SOURCEDIR=$@ BUILDDIR=$(CURR_DIR)/build/$* EXECUTABLE=$(CURR_DIR)/dist/$* ARCH=$(ARCH)
	@echo
...
$(PROGS): % : $(CURR_DIR)/src/%/makefile
	...
	@cd $(CURR_DIR)/src/$@ && make $(CTYPE) SOURCEDIR=$(CURR_DIR)/src/$@ BUILDDIR=$(CURR_DIR)/build/$@ EXECUTABLE=$(CURR_DIR)/dist/$@ ARCH=$(ARCH)
	@echo
